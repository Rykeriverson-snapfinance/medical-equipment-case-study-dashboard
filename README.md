...
- Row grain: One row per prequalification application; one repayment row per funded account in the repayment extract
...
- Source of truth: `data/case_study_dataset.csv` and `data/repayment_results.csv`
...
Recommendation: continue the program selectively, focus expansion on stronger merchants and lower-risk segments, and use repayment outcomes as expansion guardrails for F-grade and swap-in exposure.

## Required Caveat

The repayment extract adds missed-payment, delinquency, charge-off, early-payoff, net funded amount, projected amount paid, and account-status fields. Profit proxy is still not fully loaded margin because it excludes cost of capital, servicing cost, realized recoveries, and loss timing.
...
- `data/repayment_results.csv`: Local repayment extract joined to completed application records.
...
Keep the dashboard contract aligned with any metric, filter, source, or recommendation change. If richer account economics become available, add them through `data.py`, document the new source and grain in `dashboard_contract.yaml`, and update the Remaining Caveats tab.
